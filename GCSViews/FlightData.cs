...
namespace MissionPlanner.GCSViews
{
    public partial class FlightData : MyUserControl, IActivate, IDeactivate
    {
        ...
        Color[] colorsForDefaultQuickView = new Color[] { Color.White }; // new Color[] { Color.Blue, Color.Yellow, Color.Pink, Color.LimeGreen, Color.Orange, Color.Aqua, Color.LightCoral, Color.LightSteelBlue, Color.DarkKhaki, Color.LightYellow, Color.Violet, Color.YellowGreen, Color.OrangeRed, Color.Tomato, Color.Teal, Color.CornflowerBlue };
        ...
        public enum actions
        {
            Preflight_Calibration, // UROC: moved preflight cal to top so it is the default display
            Loiter_Unlim,
            Return_To_Launch,
            Mission_Start,
        ...
        // +++++++++++ UROC: BEGIN added method for debug logging ++++++++++++++
        public static class DebugLog
        {
            private static readonly string logFile = System.IO.Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), "mp_debug_log.txt");
        ...
        // +++++++++++ UROC: END added method for debug logging ++++++++++++++
        ...
        private void loadTabControlActions()
        {
            ...
            foreach (var tabname in tabarray)
            {
                ...
                foreach (TabPage tabPage in TabListOriginal)
                {
                    ...
                    {
                        // UROC: added if statement to catch quick page crashing and safely add it back
                        if (tabQuickDetached && tabPage == tabQuick)
                        {
                            DebugLog.Log("skipping activating quick tab bc detached");
                            added = true;
                            continue;
                        }
        ...
        public void updateDisplayView()
        {
            ...
            {
                tabControlactions.TabPages.Add(tabActions);    // UROC: changed default tab to actions incase Quick is undocked
                tabControlactions.SelectedIndex = 1;           // UROC: changed default to actions index
            }
        ...
        // UROC removed ground color strip menu option
        private void groundColorToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (groundColorToolStripMenuItem.Checked)
            {
                ...
                // UROC Custom Color
                hud1.groundColor1 = Color.FromArgb(64, 64, 65);
                hud1.groundColor2 = Color.FromArgb(64, 64, 65);
            }
            else
            {
                // green
        ...
        {
            // =========== UROC: BEGIN commenting out previous undock method contents ==============
            //(sender as Form).SaveStartupLocation();
            ...
            // =========== UROC: END commenting out previous undock method contents ==============

            // +++++++++++ UROC: BEGIN changes to hopefully stop quick page crashing when detached +++++++++++
            if (sender is Form form)
                form.SaveStartupLocation();

            // only reattach if FlightData is still alive
            if (!IsDisposed && tabControlactions != null && !tabControlactions.IsDisposed)
            {
                if (tabQuick != null && !tabQuick.IsDisposed)
                {
                    if (!tabControlactions.Controls.Contains(tabQuick))
                        tabControlactions.Controls.Add(tabQuick);

                    tabControlactions.SelectedTab = tabQuick;
                }

                if (contextMenuStripQuickView != null && !contextMenuStripQuickView.IsDisposed)
                {
                    var undockItem = contextMenuStripQuickView.Items["undockToolStripMenuItem"] as ToolStripMenuItem;
                    if (undockItem != null)
                        undockItem.Visible = true;
                }
            }

            tabQuickDetached = false;

            // +++++++++++ UROC: END changes to hopefully stop quick page crashing when detached +++++++++++
        }
